...
exports.show = function(req, res,callback) {
	if(db.mongoose.Types.ObjectId.isValid(req.param('uuid')) && req.param('uuid').match(/^[0-9a-fA-F]{24}$/)) {
		Model.findOne( { _id: db.mongoose.Types.ObjectId(req.param('uuid')) }, function (err, detail) {
		if(!err) {
			if(callback) {
				callback(detail);
			}
		...
		Model.findOne({ filename: req.param('uuid') }, function (err, detail) {
		if(!err) {
			if(callback) {
				callback(detail);
			}
...
exports.findDetail = function(data,cb) {
	...
	data.forEach(function(d){
		var t=d.filename||d.photoPath;
		Model.findOne({filename:decodeURI(t)},function(err,detail){
			e.push(detail);
