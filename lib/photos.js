		...
		if (!err){
			...
			files.forEach(function (file) {
				...
				if (rFilter.test(file.path)) {
					...
					tmpResult["imagePath"] = SYSTEM_DELIMITER+"thumb"+SYSTEM_DELIMITER+filename+SYSTEM_DELIMITER+'full_'+filename;
...
exports.show = function(req, res,callback) {
	Model.findOne({filename:req.params.uuid},function(err,detail){
		if(!err && callback) callback(detail);
	});
};
