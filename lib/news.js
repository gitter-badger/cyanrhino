...
exports.post = function(req, res,callback) {	
	new Model({title: req.body.title, author: req.user.username,content: req.body.content,category:req.body.category }).save(function(err, todo, count){
...
exports.list = function(callback) {
	Model.find({}).where('category').ne('career').exec(function(err, docs) {
		if (!err){ 
...
exports.doUpdate=function(req,res,callback){
	Model.update({ _id: req.params._id }, { $set: { title: req.body.title,content:req.body.content, author:req.user.username,category:req.body.category }},
	function (err) {
		if (!err){ 
