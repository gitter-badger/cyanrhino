		...
		if (!err){
			...
			files.forEach(function (file) {
				...
				if (rFilter.test(file.path)) {
					...
					tmpResult["path"] = SYSTEM_DELIMITER+"thumb"+SYSTEM_DELIMITER+filename+SYSTEM_DELIMITER+file.getLayout(file.layout)+'_128_'+filename;
					tmpResult['index']=file.index;
					tmpResult['layout']=file.getLayout(file.layout);
					tmpResult['realPath']=SYSTEM_DELIMITER+"thumb"+SYSTEM_DELIMITER+filename+SYSTEM_DELIMITER+file.getLayout(file.layout)+'_'+filename;
