...
SYSTEM_DELIMITER=os.type().indexOf('Windows')!==-1?'\\':'/';


function processImage(file,cropping, bannersize,cb) {
	im.identify(file.path.slice(0,file.path.lastIndexOf(SYSTEM_DELIMITER)+1)+file.name, function(err, features) {
		if (err&&cb) {
			cb(err);
			console.log(err);
			return;
		...
		var image = features;
		image.url = file.path.slice(0,file.path.lastIndexOf(SYSTEM_DELIMITER)+1)+file.name;
		image.previews = {};
		...
		function getPreviewPath(size) {
			var dirPath = process.cwd()+SYSTEM_DELIMITER+'preview'+SYSTEM_DELIMITER+file.name+SYSTEM_DELIMITER+size;
			return dirPath+SYSTEM_DELIMITER+file.name;
		}
		...
		function resize(size) {			
			var previewPath = dirPath+SYSTEM_DELIMITER+bannersize+'_'+size+'_'+file.name;
			im.resize({
				srcPath: dirPath+SYSTEM_DELIMITER+bannersize+'_'+file.name,
				dstPath: previewPath,
				width:   size,
				height:  ''
		...
		var dirPath = process.cwd()+SYSTEM_DELIMITER+'preview'+SYSTEM_DELIMITER+file.name;
		try {
		...
		im.convert([ file.path.slice(0,file.path.lastIndexOf(SYSTEM_DELIMITER)+1)+file.name, 
		"-crop", 
		cropping.w+"x"+cropping.h+"+"+(cropping.x<0?0:cropping.x)+"+"+(cropping.y<0?0:cropping.y), 
		dirPath+SYSTEM_DELIMITER+bannersize+'_'+file.name
		], function(err) { 
			if(err) { 
				cb(err); 
			}else{
...
exports.upload=function(req,res){
	...
	var form = new formidable.IncomingForm({
		...
		encoding:'utf-8',
		multiples:true,
		uploadDir:process.cwd()+SYSTEM_DELIMITER+'uploads'
	}),
	fields = [],
	files = [];
	...
	form.on ('fileBegin', function(name, file){
		file.path = form.uploadDir + SYSTEM_DELIMITER + file.name;
	});
...
exports.legacyUpload=function(req,res){
	...
	files.forEach(function(file){
		...
		fs.rename(
			file.path,
			process.cwd() + SYSTEM_DELIMITER+"uploads"+SYSTEM_DELIMITER+file.name,
			function(error) {
				if(error) {
					res.send({
				...
				photoService.save({
					...
					content: '',
					tag: '',
					path:process.cwd() + SYSTEM_DELIMITER+"uploads"+SYSTEM_DELIMITER+file.name
				});
